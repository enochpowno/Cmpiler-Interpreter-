...
public class Visitor1 extends sklLanguageBaseVisitor<Object> {

    List<Token> tokenList = new ArrayList<>();
    public String IDENTIFIER = "[a-zA-Z][a-zA-Z0-9_]*";
    public static ArrayList<String> data_types = new ArrayList<>(Arrays.asList("int", "float", "boolean", "String", "char"));
    ...
    Stack<Scope> stack; // monitors where the stack being executed is
    mainIDE mainController;
    Stack<Symbol> globalVar; // list of global variables
    String str;
    String methodHold;
    ArrayList<Symbol> symbolHolder = new ArrayList<Symbol>(); // for local variable
    ...
    public Visitor1(List<Token> tokens, sklLanguageLexer lexer, Stack<Scope> stack1, Stack<Scope> scopeList, mainIDE cont) {
        stack = stack1;
        lexerList = lexer;
    ...
    public Object visitMethodDeclaration_1(sklLanguageParser.MethodDeclaration_1Context ctx) {
        ...
        // semantic checking if function has return statement
        if (data_types.contains(temp.get(0).getText())) { // if first token is a data type of int, float, etc
            ...
            if (!pass) {
                mainController.appendSemanticError("Syntax Error: Missing return statement in function '" + ctx.IDENTIFIER() + "' at line " + ctx.getStart().getLine());
            }
        } else { // if first token is void, there should be no pass statement
            for (int i = 1; i < temp.size(); i++) { // loop that there should be a pass statement
                if (temp.get(i).getText().equals("return")) {
                    pass = true;
                    break;
            ...
            if (pass) {
                mainController.appendSemanticError("Semantic Error: Unexpected return statement in function '" + ctx.IDENTIFIER() + "' at line " + ctx.getStart().getLine());
            }
        }
        ...
        if (!duplicate) {
            ...
            scopeList.add(add); // adds all methods to ARRAYLIST
        } else {
            mainController.appendSemanticError("Semantic Error: Duplicate function '" + ctx.IDENTIFIER() + "' at line " + ctx.getStart().getLine());
    ...
    public Object visitMethodDeclaration_2(sklLanguageParser.MethodDeclaration_2Context ctx) {
        ...
        mainController.appendSemanticError("Semantic Error: Missing return type at line " + ctx.getStart().getLine());
        return visitChildren(ctx);
    }
            ...
            for (int i=0; i<ctx.getChildCount(); i++){
                if(ctx.getChild(i) instanceof sklLanguageParser.Statement_2Context){
                    ...
                    if(ctx.getChild(i+2) != null){
                        mainController.appendSemanticError("Semantic Error: Unreachable Code");
                    }
                    break;
                }
                else{
                ...
                if(indexError) { //removed comment for main controller
                	mainController.appendSemanticError("Runtime Error: Index out of bounds at line " + lineError);
                    indexError = false;
                }
                else if(nullError) {
                	mainController.appendSemanticError("Runtime Error: Null Pointer exception at line " + lineError);
                    nullError = false;
                }
                else if(arithmeticError) {
                	mainController.appendSemanticError("Runtime Error: Arithmetic Exception at line " + lineError);
    ...
    public Object visitFieldDeclaration(sklLanguageParser.FieldDeclarationContext ctx) {
        if (ctx.getText().contains("<missing ';'>"))
            mainController.appendSemanticError("Syntax Error: Missing ';' at line " + ctx.getStart().getLine());

        if (!ctx.getText().contains("=")) { // if variable declaration has no initialization. Ex. number x;
            mainController.appendSemanticError("Semantic Error: Variable not initialized at line " + ctx.getStart().getLine());
    ...
    public Object visitVariableDeclarator(sklLanguageParser.VariableDeclaratorContext ctx) {
        ...
        if (ctx.parent.parent.parent.getRuleContext().getClass().toString().contains("ForInitContext")) { // checks if the child of for loop is a variable declarator
            if (ctx.getChildCount() == 1) { // variable declarator only has 1 child
                mainController.appendSemanticError("Syntax Error: Missing assignment operator at line " + ctx.getStart().getLine());
            }
        }
        ...
        for (int i = 0; i < ctx.getChildCount(); i++) {
            ...
            if (ctx.getChild(i) instanceof sklLanguageParser.VariableDeclaratorIdContext) {
                ...
                if (ctx.getChildCount() >= 3 && ctx.getChild(i + 2).getChild(0) instanceof sklLanguageParser.InputCorrectContext) {
                    ...
                    if (!stack.isEmpty()) {
                        ...
                        mainController.appendSemanticError("Semantic Error: Cannot ask for input globally");
                    }
    ...
    public String processExpressionPlease(ParseTree newCtx){
        System.out.println("PROCESS PLSSSSSS" + newCtx.getText() + newCtx.getClass());
        str = "";
        ...
        if(newCtx instanceof sklLanguageParser.Expression_3Context){
            ...
            str = temp; // str was reset when process expression please was called

            System.out.println("I AM PRINTING AN ARRAY = " + arrIndex);
            System.out.println();
            Object valueInArrIndex = "";

            // possible null pointer and index out of bounds
            try {
                ...
                if (tempVal == null){
                    ...
                    System.out.println("DI pala null eh = " +tempVal);
                    str += tempVal;
                }
            ...
            if (newCtx.getChild(0) instanceof sklLanguageParser.Primary_1Context){
                ...
                try {
                    ...
                    mainController.appendSemanticError("Semantic Error: Undeclared Variable at line " + ((sklLanguageParser.Expression_1Context) newCtx).getStart().getLine());
                }
            }
            else {
        ...
        if (str.contains("\"") || str.contains("\'")) {
            ...
            System.out.println("WHATS YOUR PROBLEM =>" + str);
            try {
    ...
    public void processChild2(ParseTree newCtx){
        ...
        if(newCtx instanceof sklLanguageParser.Expression_3Context){
            ...
            System.out.println("returned val from method call is = " + getVal4);
            if(getVal4 != null){
                str = temp4;
                System.out.println("str after method call = " + str);
                str += getVal4;
...
//                            mainController.appendSemanticError("Semantic Error: Duplicate variable declaration at line " + ctx.getStart().getLine());
                        }
                    }
                }
            }
                ...
                if (!stack.isEmpty()) {
                    ...
                    while (valid) {
                        ...
                        try {
                            ...
                            // run time Arithmetic error
//                            mainController.appendSemanticError(comparison + "Error: RunTimeException - ArithmeticException ");
                            return null;
                        }
                        ...
                        for (int z = 0; z < tokens.size(); z++) {
                            ...
                            if (checkTokenType(tokens.get(z).getText()).equals("IDENTIFIER")) {
                                try {
                                    ...
                                    mainController.appendSemanticError("Semantic Error: Undeclared Variable at line " + ((sklLanguageParser.ParExpressionContext) ctx.getChild(i)).getStart().getLine());
                                    return null;
                                }
...
//                            mainController.appendSemanticError(comparison + "Error: RunTimeException - ArithmeticException ");
    ...
    public Object visitStatement_8(sklLanguageParser.Statement_8Context ctx) {
        ...
        if (checkTokenType(cut).equals("KEYWORD"))
            mainController.appendSemanticError("Syntax Error: Expected IDENTIFIER at line " + ctx.getStart().getLine());
    ...
    public Object visitCorrectFunc(sklLanguageParser.CorrectFuncContext ctx) {
        ...
        for (Scope item : scopeList) {
            System.out.println("SCOPELIST + " + item.getName() + " vs " + ctx.IDENTIFIER().getText() + funcFound);
            ...
            if (ctx.IDENTIFIER().getText().equals(item.getName())) {
                ...
                // TODO place all parameters of CORRECT FUNC TO NULL
        ...
        if(!funcFound){
            mainController.appendSemanticError("Semantic Error: Function not defined at line " + ctx.getStart().getLine());
            return null;
        }
        ...
        if (!stack.isEmpty()){
            if (ctx.getChildCount() == 3) {// if no parameter
                ...
                while (hmIterator.hasNext()) {
                    ...
                    if (symbol.getValue() == null) {
                        System.out.println();
                        mainController.appendSemanticError("Semantic Error: Missing parameters at line " + ctx.getStart().getLine());
                        return null;
                    }
                }
                ...
                for (int i = 0; i < ctx.getChildCount(); i++) {
                    ...
                    if (ctx.getChild(i) instanceof sklLanguageParser.ExpressionListContext) {
                        ...
                        for (int j = 0; j < ctx.getChild(i).getChildCount(); j++) {
                            // TODO PART IS UNSTABLE ESP IF THERE IS A FUNCTION
                            if (ctx.getChild(i).getChild(j) instanceof sklLanguageParser.Expression_10Context | ctx.getChild(i).getChild(j) instanceof sklLanguageParser.Expression_1Context) { // do flattokens then EvalEx concat str
                                ...
                                for (int x = 0; x < tempExpression.size(); x++) {
                                    if (checkTokenType(tempExpression.get(x).getText()).equals("IDENTIFIER")) {
                                        try {
                                            ...
                                            //str += getSymbolValue(stack.peek(), stack.peek().getSymbolTable(), tempExpression.get(x).getText()).toString() + " ";
                                        } catch (Exception e) {
                                ...
                                if(str.contains("\"")){// if string literal pinass,
//                                    Expression expression = new Expression(str);
//                                    BigDecimal val = expression.eval();
//                                    value = val.setScale(4);
                                ...
                                }else{// if what's passed to the parameter is not a string literal like var or decimal
                                    Expression expression = new Expression(str);
                                    BigDecimal val = expression.eval();
                                    value = val.setScale(2, BigDecimal.ROUND_HALF_DOWN).toString();
                                }
                                ...
                                String tempVarn;
                                try{
                                    tempVarn = stack.peek().getTreeOfFunction().getParent().getChild(2).getChild(1).getChild(j).getChild(1).getText();
                                    ...
                                    if (stack.peek().getSymbolTable().get(tempVarn).getData_type().equals("int")){
                                        ...
                                        try{
                                            ...
                                            mainController.appendSemanticError("Semantic Error: Parameter Data type mismatch for function call at line " + ctx.getStart().getLine());
                                        }
                                    ...
                                    mainController.appendSemanticError("Semantic Error: Extra parameters at line " + ctx.getStart().getLine());
                                }
                            }
                        }
                    }
                ...
                if (checkTokenType(ctx.parent.parent.getChild(0).getText()).equals("IDENTIFIER")) {
                    ...
                    if (checkTokenType(ret + "").equals("DECIMAL_LITERAL")) { // object returned by the function
                        if (sym.getData_type().equals("int")) {
                            ...
                            mainController.appendSemanticError("Semantic Error: Return type mismatch at line " + ctx.getStart().getLine());
    ...
    public Object visitWrongFunc(sklLanguageParser.WrongFuncContext ctx) {
        mainController.appendSemanticError("Syntax Error: Redundant parentheses at line " + ctx.getStart().getLine());
        return visitChildren(ctx);
    }
    ...
    public Object visitExpression_17(sklLanguageParser.Expression_17Context ctx) {
        ...
        if(ctx.getChild(0) instanceof sklLanguageParser.Expression_3Context){
            ...
            try {
                ...
                mainController.appendSemanticError("Semantic Error: Invalid array index at line " +ctx.getStart().getLine());
                return null;
            }
...
//                        mainController.appendSemanticError("Error: Input data type mismatch at line " + ctx.getStart().getLine());
                        return null;
                    }
...
//                        mainController.appendSemanticError("Error: Input data type mismatch at line " + ctx.getStart().getLine());
    ...
    public Object visitExpression_10(sklLanguageParser.Expression_10Context ctx) {
        ...
        if(ctx.getChild(0) instanceof sklLanguageParser.Expression_3Context){
            ...
            if(getVal != null){
                ...
                System.out.println("str after method call = " + str);
                str += getVal;
                lineError = ((sklLanguageParser.Expression_4Context) ctx.getChild(0)).getStart().getLine();
            }
    ...
    public Object visitPrimary_1(sklLanguageParser.Primary_1Context ctx) {
        ...
        if (ctx.getText().contains("<missing ')'>"))
            mainController.appendSemanticError("Syntax Error: Uneven parenthesis. Duplicate ‘(’ at line " + ctx.getStart().getLine());
    ...
    public Object visitLiteralError(sklLanguageParser.LiteralErrorContext ctx) {
        ...
        //  System.out.println("VISIT LITERAL ERROR");
        for (int i = 0; i < ctx.getChildCount(); i++) {
            if (ctx.getChild(i).getText().equals("++")) {
                mainController.appendSemanticError("Syntax Error: Duplicate + sign at line " + ctx.getStart().getLine());
...
//    @Override public Object visitOutputBlank(sklLanguageParser.OutputBlankContext ctx) {
//        return visitChildren(ctx);
//    }
...
//        System.out.println("===================================================");
    ...
    public float makeFloat(Object val) {
    ...
    public Object visitOutputExtraPlus(sklLanguageParser.OutputExtraPlusContext ctx) {
        mainController.appendSemanticError("Syntax Error: Additional ‘+’ sign at line " + ctx.getStart().getLine());
        return visitChildren(ctx);
    }
    ...
    public Object visitOutputMissingPlus(sklLanguageParser.OutputMissingPlusContext ctx) {
        ...
        if (ctr % 2 == 0) {
            mainController.appendSemanticError("Syntax Error: Missing + sign at line " + ctx.getStart().getLine());
        } else {
...
//        }
    ...
    public boolean checkVariableDuplicateinGlobalVar(String var_name, int line) {
        ...
        for (Symbol item : globalVar) {
            if (item.getGlobal_var_name().equals(var_name)) {
                mainController.appendSemanticError("Semantic Error: Duplicate global variable " + var_name + " at line " + line);
                return false;
            } else {
                return true;
            }
    ...
    public boolean checkVariableDuplicateinScope(Scope scope, String var_name, int line) {
        ...
        if (scope.getSymbolTable().containsKey(var_name)) {
            mainController.appendSemanticError("Semantic Error: Duplicate local variable " + var_name + " at line " + line);
            return false;
        } else {
            System.out.println(var_name + " IS A VALID VARIABLE NAME!");
            return true;
        }
    }
